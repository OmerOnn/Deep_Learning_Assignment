...
MODE = "semihard"   # "random" or "semihard"
MARGIN = 0.2
CKPT_PATH = f"checkpoints/koch_triplet_{MODE}_m{MARGIN}_best.pt"

RESULTS_DIR = "results"
SCORES_CSV = os.path.join(RESULTS_DIR, f"koch_triplet_{MODE}_m{MARGIN}_test_scores.csv")
ROC_PNG = os.path.join(RESULTS_DIR, f"koch_triplet_{MODE}_m{MARGIN}_roc.png")
SUMMARY_TXT = os.path.join(RESULTS_DIR, f"koch_triplet_{MODE}_m{MARGIN}_summary.txt")
MISCLS_TXT = os.path.join(RESULTS_DIR, f"koch_triplet_{MODE}_m{MARGIN}_misclassified.txt")
...
def predict_scores_from_distance(model, loader, device="cpu"):
    ...
    for batch in loader:
        ...
        score = (-dist).detach().cpu().numpy()      # higher = more likely same

        all_scores.append(score)
        all_labels.append(label.detach().cpu().numpy().astype(int))
...
def main():
    ...
    # val split from train PAIRS for threshold selection
    full_train_pairs = LFWDataset(PAIRS_TRAIN, LFW_ROOT, transform=transform, return_paths=False)
    n_total = len(full_train_pairs)
    n_val = int(n_total * VAL_FRAC)
    n_train = n_total - n_val
    gen = torch.Generator().manual_seed(SEED)
    _, val_ds = random_split(full_train_pairs, [n_train, n_val], generator=gen)
    ...
    # ROC plot
    plt.figure()
    plt.plot(fpr, tpr, label=f"Triplet {MODE} m={MARGIN} (AUC={test_auc:.3f})")
    plt.plot([0, 1], [0, 1], linestyle="--")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title(f"ROC Curve - Koch Triplet ({MODE}, m={MARGIN})")
