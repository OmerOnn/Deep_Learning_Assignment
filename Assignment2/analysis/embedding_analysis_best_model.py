...
def collect_images_by_identity():
    """
    Collect images from identities that have at least MIN_IMAGES_PER_IDENTITY images.
    This allows t-SNE to show multiple points per identity instead of one point only.
    """
    identities = {}
    ...
    for identity in sorted(os.listdir(IMAGES_ROOT)):
        ...
        images = [
            os.path.join(identity_dir, file_name)
            for file_name in sorted(os.listdir(identity_dir))
...
def select_identities(identities):
    """
    Select NUM_IDENTITIES identities and up to MAX_IMAGES_PER_IDENTITY images per identity.
    """
    random.seed(RANDOM_SEED)

    eligible_names = list(identities.keys())
    random.shuffle(eligible_names)

    selected_names = eligible_names[:NUM_IDENTITIES]

    selected = {}

    for identity in selected_names:
        images = identities[identity].copy()
        random.shuffle(images)
        selected[identity] = images[:MAX_IMAGES_PER_IDENTITY]
...
def compute_embeddings(model, selected_identities):
    ...
    for identity, image_paths in selected_identities.items():
        for image_path in image_paths:
            embedding = embed_image(model, image_path)
...
def normalize_embeddings(embeddings):
    """
    Clean and L2-normalize embeddings to make t-SNE more numerically stable.
    """
...
def plot_tsne(embeddings, labels):
    ...
    plt.title(
        f"t-SNE of Test Embeddings ({len(unique_labels)} identities, {n_samples} images)"
    )
    plt.xlabel("t-SNE-1")
    plt.ylabel("t-SNE-2")
...
def compute_intra_inter_distances(embeddings, labels):
    ...
    intra-class:
        distance between two images of the same identity

    inter-class:
        distance between two images from different identities
    """
    embeddings = normalize_embeddings(embeddings)
...
def main():
    ...
    identities = collect_images_by_identity()

    print(
        f"Found {len(identities)} identities with at least "
        f"{MIN_IMAGES_PER_IDENTITY} images"
    )

    selected_identities = select_identities(identities)

    total_images = sum(len(images) for images in selected_identities.values())
