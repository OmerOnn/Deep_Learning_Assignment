...
def main():
    ...
    parser.add_argument("--koch_fc", type=int, default=1024)
    ...
    OUT = (
        f"results/experiment2_backbone/"
        f"{args.backbone}_triplet_semihard_m{args.margin}/"
        f"emb{args.emb_dim}_{run_id}"
    )
    ...
    log(
        f"Config: backbone={args.backbone}, epochs={args.epochs}, "
        f"lr={args.lr}, margin={args.margin}, "
        f"P={args.P}, K={args.K}, seed={args.seed}, "
        f"emb_dim={args.emb_dim}, koch_fc={args.koch_fc}"
    )
    ...
    if args.backbone == "koch":
        backbone = KochBackbone(
            fc_units=args.koch_fc,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for b, (imgs, labels) in enumerate(loader):
            ...
            if b % 10 == 0:
                log(
                    f"[{args.backbone.upper()}] epoch {epoch}/{args.epochs} "
